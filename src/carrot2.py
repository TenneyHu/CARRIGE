...
def norm(score, min_score=-10.0, max_score =6.0):
...
class CarrotRetriever():
    def __init__(self, emb_model, index_dir, full_document_maps, debugging = 0, 
        ...
        self.history_generated_results = []

    def update_history_generated_results(self, inputs):
        self.history_generated_results = inputs
    ...
    def reranking(self, query, res_list):
        ...
        if self.reranking_type == "MMR" or self.reranking_type == "MMR*":
            selected_vectors = []
            history_vectors = []
                ...
                for i in remaining_indices:
                    ...
                    if selected_vectors != []:
                        sim_to_selected = cosine_similarity(
                            ...
                            np.stack(selected_vectors)).max() 

                    if history_vectors != []:
                        sim_to_history = cosine_similarity(
                            embeddings[i].reshape(1, -1),
                            np.stack(history_vectors)
                        ).max() 
                  
                    mmr_score = lambda_param * relevance - (1 - lambda_param) * sim_to_selected - beta * sim_to_history

                    mmr_scores.append((mmr_score, relevance, sim_to_selected, sim_to_history, i))
                selected_mmr_score, selected_relevance, selected_sim_to_selected, selected_sim_to_history, selected_idx = max(mmr_scores, key=lambda x: x[0])
                #print (res_list[selected_idx].title, selected_mmr_score, selected_relevance, selected_sim_to_selected, selected_sim_to_history, selected_idx)
                #print (self.history_generated_results)
